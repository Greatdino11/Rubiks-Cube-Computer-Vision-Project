...
# Initial state of the Rubik's cube (all sides are white)
state=  {
...
# Mapping of colors to their corresponding symbols
sign_conv = {
    'green'  : 'F',
    'white'  : 'U',
    'blue'   : 'B',
...
# Define the coordinates of stickers on different faces of the cube
stickers = {
        'main': [
...
# Define font and text positions for labeling faces
...
check_state = []  # List to keep track of scanned sides
solution = []  # List to store the solution steps
solved = False  # Flag to indicate if the cube is solved

cap = cv2.VideoCapture(0)  # Open the default camera
cv2.namedWindow('frame')  # Create a window to display the frame

def rotate(side):
    """
    Rotate the specified side of the Rubik's cube clockwise.
    Updates the state of the cube accordingly.
    """
    main=state[side]
    front=state['front']
    left=state['left']
...
def revrotate(side):
    """
    Rotate the specified side of the Rubik's cube counter-clockwise.
    Updates the state of the cube accordingly.
    """
    main=state[side]
    front=state['front']
    ...
    # Rotate the main side counter-clockwise
...
def color_detect(h,s,v):
    """
    Detect the color based on the given HSV values.
    ...
    if h < 10 and s>5:
        ...
        return 'orange'
    elif h <= 39 and h>15 and v<220:
        return 'yellow'
    elif h>=40 and h<= 70:
        return 'green'
    elif h <= 130 and h>70 and s>70:
        return 'blue'
    elif v>220 and h>10 and h<70 and s<5:
        return 'white'
...
def draw_stickers(frame,stickers,name):
        """
        Draw rectangles around the stickers on the specified face of the cube.
        """
...
def draw_preview_stickers(frame,stickers):
        """
        Draw rectangles around the preview stickers for all faces of the cube.
        """
...
def texton_preview_stickers(frame,stickers):
        """
        Draw text labels on the preview stickers for all faces of the cube.
        """
        stick=['front','back','left','right','up','down']
        for name in stick:
            for x,y in stickers[name]:
...
def fill_stickers(frame,stickers,sides):
    """
    Fill the stickers on the specified faces of the cube with their respective colors.
    """
...
def process(operation):
    """
    Process the solution moves and update the cube state accordingly.
    Displays the solution steps on the preview window.
    """
    replace={
                "F":[rotate,'front'],
...
if __name__=='__main__':
    ...
    while True:
        ...
        for x,y in stickers['current']:
            ...
            cv2.rectangle(img,(x,y),(x+20,y+20),color[color_name],-1)
            a+=1
        ...
        if k == 27:
            ...
            if len(set(check_state))==6:    
                try:
                    ...
                    print("error in side detection ,you may do not follow sequence or some color not detected well. Try again", e)
            else:
